---
title: "Weekly arXiv Digest — 2026-06-15–2026-06-19"
week: 2026-0615-0621
date_range: ["2026-06-15", "2026-06-19"]
tags: [mechanistic-interpretability, reasoning-rl, quantization-precision, safety-alignment, self-evolving-agents, evaluation-benchmarks, training-stability]
papers: 216
---

## 本周必读 / Must Read This Week

> 本周 216 篇去重论文中精选 8 篇。优先纳入跨日复现（06-18/06-19 双日命中）与各日 daily must-read。/ Eight picks from 216 deduped papers, prioritizing cross-day recurrence and daily must-reads.

### [[2606.20075]] What Makes Effective Supervision in Latent Chain-of-Thought: An Information-Theoretic Analysis
**推荐理由**：把潜 CoT 监督失败重构为"双重坍缩"（沿优化路径的梯度衰减 + 潜空间流形漂移），用统一潜探针（ULP）度量潜状态与显式推理步的互信息，证明推理精度被潜链保留的互信息严格上界。
**Why read**: Reframes latent-CoT supervision failure as "dual collapse"; the Unified Latent Probe shows reasoning accuracy is strictly bounded by the mutual information retained in the latent chain. *(跨日复现 06-18/06-19)*

### [[2606.20128]] The Correctness Illusion in LLM-Generated GPU Kernels
**推荐理由**：用种子模糊测试证明 KernelBench/TritonBench 的单形状 allclose 预言机会把"转录错误"的有 bug 内核误判为正确；种子预言机零误报抓出 10/10"正确性幻觉"，每次失败都可从存储种子逐字节复现。
**Why read**: Seeded fuzzing exposes that single-shape allclose oracles mis-certify buggy LLM-generated kernels; the seeded oracle catches all 10 "correctness illusions" with zero false positives. *(跨日复现 06-18/06-19)*

### [[2606.20381]] Rethinking Shrinkage Bias in LLM FP4 Pretraining: Geometric Origin, Systemic Impact, and UFP4 Recipe
**推荐理由**：推导 FP4(E2M1) 在 RNE 下"收缩偏差"的几何起源，它跨层乘性累积并被 RHT 放大——这是 NVFP4 训练不稳定的统一根因；UFP4 配方在 Dense 1.5B / MoE 7.9B / 124B 预训练上显著降低 BF16 相对损失误差。
**Why read**: Derives the geometric origin of FP4 shrinkage bias that accumulates across layers — the unifying root cause of NVFP4 instability; UFP4 cuts loss error across 1.5B–124B pretraining. *(跨日复现 06-18/06-19)*

### [[2606.19236]] STARE: Surprisal-Guided Token-Level Advantage Reweighting for Policy Entropy Collapse
**推荐理由**：给出 GRPO 熵坍缩最干净的机制诊断（优势–惊讶度四象限结构），证明近临界性结果 W*−1 = O(T⁻¹)，在 1.5B–32B 模型上 AIME24/25 +4–8%。
**Why read**: Cleanest mechanistic diagnosis of GRPO entropy collapse via a four-quadrant advantage–surprisal structure; +4–8% on AIME24/25 across 1.5B–32B models.

### [[2606.17478]] StateWitness
**推荐理由**：把对目标 LLM 隐藏状态的标量探针升级为可查询、schema 化的独立审计解码器，返回校准分数与可检证据；在 2 目标 × 7 欺骗基准上平均 AUROC 0.916，比最强黑箱监视器高 11.6%、比最强线性探针高 25.0%。
**Why read**: Upgrades scalar hidden-state probes into a queryable, schema-formatted audit decoder; mean AUROC 0.916 across 7 deception benchmarks, +11.6% over the best black-box monitor.

### [[2606.18656]] The Wrong Kind of Right: Quantifying and Localizing Misfired Alignment
**推荐理由**：形式化"误触发对齐"——对齐后的 LLM 在刻板印象相关问题上自信地答错；25 个模型全部误触发（最高 18.9%），失败精准定位到后层"否决"注意力头。
**Why read**: Formalizes "misfired alignment" — aligned LLMs confidently answer wrong on stereotype items; all 25 models misfire (up to 18.9%), localized to late-layer "veto" attention heads.

### [[2606.18874]] Externalizing Research Synthesis and Validation in AI Scientists (Xcientist)
**推荐理由**：把 AI 科学家的不透明研究链外化为契约治理的产物，将"声明漂移（claim drift）"形式化为一等失败模式，在三组案例研究上超越 AI-Scientist-v2。
**Why read**: Externalizes the AI scientist's opaque research chain into contract-governed artifacts, making "claim drift" a first-class failure mode; outperforms AI-Scientist-v2 on three case studies.

### [[2606.17830]] Functional Equivalence in Attention
**推荐理由**：形式化注意力与位置编码的函数等价对称群，严格证明 sinusoidal PE 保持 vanilla 注意力的完整对称群，而 RoPE 将其严格缩小为 S_h × H(d_h) × GL(d_h)，为 RoPE 的实际主导地位给出原则性解释；附带的 LMC 研究是重要的负面结果。
**Why read**: Formalizes the functional-equivalence symmetry group of attention+PE; proves RoPE strictly reduces it vs sinusoidal, giving a principled explanation for RoPE's dominance.

## 本周主题脉络 / Themes This Week

### 1. 机制可解释性与表征几何 / Mechanistic Interpretability & Representation Geometry
本周可解释性工作普遍呈现"配对正/负结果"与"几何化"两条主线：既定位可线性解码的方向，也坦承条件残差化后这些方向消失的边界。从代码正确性预生成探针（AUC 0.931 但 repair 方向消失）、到 VLM"凝视头"、到多语义叠加的几何约束，一条"表征能被读到几分"的脉络贯穿全周。
Representative: [[2606.14530]] [[2606.17478]] [[2606.18656]] [[2606.19831]] [[2606.19946]] [[2606.14703]]

### 2. 推理、强化学习后训练与训练动力学 / Reasoning, RL Post-Training & Training Dynamics
RL 后训练的稳定性是本周高频议题：GRPO 熵坍缩被四象限地诊断并近临界性量化、潜 CoT 监督被信息论上界、SFT+RL 流水线被证明存在"隐藏支持阻塞"。Muon 优化器在重尾谱范数下的样本复杂度理论也补上了训练侧的基础。
Representative: [[2606.19236]] [[2606.20075]] [[2606.18089]] [[2606.14560]] [[2606.19222]]

### 3. 精度、量化与计算可靠性 / Precision, Quantization & Computation Reliability
FP4 预训练的"收缩偏差"被推导出几何起源并给出 UFP4 配方，与 Averis 的"秩一均值偏置"诊断遥相呼应；而 LLM 生成 GPU 内核的"正确性幻觉"把量化/算子的正确性评估本身变成被审计对象。低精度推理（4-bit KV、sub-1-bit 草图）与精度误差干扰（PuDGhost）共同构成"数值可靠性"主线。
Representative: [[2606.20381]] [[2606.20128]] [[2603.10444]] [[2606.20474]] [[2606.19119]] [[2606.20195]]

### 4. 安全、对齐与可解释审计 / Safety, Alignment & Auditing
本周安全工作向"内部状态审计"集中：StateWitness 把探针升级为可报告解码器、误触发对齐被定位到后层否决头、涌现误对齐有了可操作激活方向、确定性封装生成模型被提出原则。一条"用内部几何读出并干预危险行为"的脉络清晰。
Representative: [[2606.17478]] [[2606.18656]] [[2606.20225]] [[2606.19753]] [[2606.19899]]

### 5. 自演化智能体与自动化研究 / Self-Evolving Agents & Automated Research
AI 科学家从"能跑通"转向"可审计"：Xcientist 把声明漂移作为一等失败模式、EurekAgent 把瓶颈从工作流转向环境工程、记忆驱动的边际优势积累与"通用智能体必须记住什么"的理论共同把"长期记忆可靠性"推到前台。
Representative: [[2606.18874]] [[2606.13662]] [[2606.20475]] [[2606.18746]] [[2606.14202]]

### 6. 评测、基准与不确定性 / Evaluation, Benchmarks & Uncertainty
评测本身的可靠性被反复质疑：22 起生产沉默失败催生 *fail-plausible* 分类法、FID 的隐藏随机性被量化、LLM-as-Judge 被当成正-未标注学习来审计、不确定性感知的细化（AURA）被提出。一条"评测器也需要被评测"的主线贯穿全周。
Representative: [[2606.14589]] [[2606.20128]] [[2606.20536]] [[2606.19057]] [[2606.19714]]
...
*本周摘要由各日 daily digest 汇总而成：[2026-06-15](../../2026-06-15/overview.md) · [2026-06-16](../../2026-06-16/overview.md) · [2026-06-17](../../2026-06-17/overview.md) · [2026-06-18](../../2026-06-18/overview.md) · [2026-06-19](../../2026-06-19/overview.md)。*
